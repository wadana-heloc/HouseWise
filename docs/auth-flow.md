...
| 422 `Password must contain ...` on signup / member create / password-update / admin member-password reset | Submitted password fails the complexity policy (≥8 chars + lower + upper + digit + special). Login intentionally skips this check so pre-policy passwords still work. | [auth/password_policy.py](../backend/app/auth/password_policy.py); applied via the shared `Password` Pydantic type in [auth/schemas.py](../backend/app/auth/schemas.py) and [household/schemas.py](../backend/app/household/schemas.py). |
| 422 on malformed email at any auth/household endpoint | Pydantic `EmailStr` rejects non-RFC-5322 input before the handler runs. | Schemas in [auth/schemas.py](../backend/app/auth/schemas.py) and [household/schemas.py](../backend/app/household/schemas.py). |
| 409 on `PATCH /me/profile` or `PATCH /household/members/{id}` when changing `email` | Target address already exists in `auth.users` | The Supabase "already registered" exception is matched and remapped to 409 in [me/router.py](../backend/app/me/router.py) and [household/router.py](../backend/app/household/router.py). Same shape as the signup duplicate path. |
| Member's email updated in `auth.users` but `/me` still shows the old one | The endpoint must explicitly `UPDATE public.users` after `admin.update_user_by_id` (CLAUDE.md §7.4). Both `PATCH /me/profile` and admin `PATCH /household/members/{id}` do this; if you see drift in a new endpoint, mirror that pattern. | [me/router.py](../backend/app/me/router.py) `patch_profile`, [household/router.py](../backend/app/household/router.py) `update_member`. |
| 422 on `PATCH /me/health-preferences` with an unknown key (e.g. `fiber`) | `ConfigDict(extra="forbid")` on `HealthPreferencesUpdate` — typos surface loudly instead of silently persisting junk into the JSONB. | [me/schemas.py](../backend/app/me/schemas.py). Add a new preference by extending the schema, no migration needed. |
