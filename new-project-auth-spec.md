...
| POST   | `/cookbook/recipes`                 | bearer       | Manual entry. `source='manual'`. Admin → `status='approved'`; family → `status='pending'`.             |
...
- **Q6 — family permissions:** what can a `family` user actually do beyond "read own household"? Until decided, RLS is "read own household, write own `display_name` only" — the 0003 `with check` blocks self-mutation of `role` and `household_id`. All other mutations go through the backend.
  ...
  - **Cookbook recipes:** any household member can create (manual, AI generate, photo extract) and list recipes. **Admin manual** entries auto-approve; **everything else** (admin AI/photo, family manual, family AI/photo) enters as `status='pending'` and requires an admin to call `POST /cookbook/recipes/{id}/approve` before it appears in the family-wide cookbook. The gate is *who* submits, not *how*. The submitter sees their own pending row; other family members do not. Admin can `?status=pending` to see the household review queue.
