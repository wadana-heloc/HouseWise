...
| POST   | `/meal-plan/{plan_id}/finalize`     | bearer:admin | Flip `status` to `'finalized'` and append the week's deduplicated ingredients to `/items` as pending. Idempotent. |
