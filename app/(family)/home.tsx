    ...
    return (
        <View style={s.greetCard}>
            ...
            <View style={s.roleBadge}>
                ...
                <Text style={s.roleText}>Family member</Text>
            </View>
            ...
            {!loading && items.length > 0 && (
                <View style={s.progWrap}>
                    ...
                    <View style={s.progLabels}>
                        <Text style={s.progLabel}>Family progress</Text>
...
function ShoppingSection({
    loading, preview, allItems, urgentCount, userId, members, onToggle, onDelete, onSeeAll,
}: {
    loading: boolean;
    preview: Item[];
    allItems: Item[];
                    ...
                    return (
                        <View key={item.id} style={[s.itemRow, i < preview.length - 1 && s.rowBorder]}>
                            ...
                            <View style={{ flex: 1, gap: 2 }}>
                                <Text style={[s.itemName, done && s.itemDone]} numberOfLines={1}>{item.name}</Text>
                            ...
                            <View style={s.itemRight}>
                                ...
                                <TouchableOpacity
                                    ...
                                    style={[s.delBtn, !own && { opacity: 0.25 }]}
...
function MyItemsSection({ items, loading }: { items: Item[]; loading: boolean }) {
    if (loading) {
        return (
            <View style={s.card}>
                {[0, 1].map((i) => (
                    <View key={i} style={[s.itemRow, i < 1 && s.rowBorder]}>
                        <Shimmer height={8} width={8} radius={4} />
                        <Shimmer height={13} width="50%" radius={5} />
                        <Shimmer height={10} width="22%" radius={4} />
                    </View>
                ))}
            </View>
        );
    ...
    if (items.length === 0) {
        return (
            <View style={s.emptyCard}>
                <Text style={s.emptyIcon}>📝</Text>
                <Text style={s.emptyTitle}>Nothing added yet</Text>
                <Text style={s.emptyBody}>Items you add will appear here</Text>
            </View>
        );
    }
            ...
            {items.map((item, i) => {
                ...
                return (
                    <View key={item.id} style={[s.itemRow, i < items.length - 1 && s.rowBorder]}>
                        <View style={[s.myItemDot, done && s.myItemDotDone]} />
                        <Text style={[s.itemName, done && s.itemDone, { flex: 1 }]} numberOfLines={1}>
                            {item.name}
                        </Text>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scrollContent}>
                ...
                <Entrance delay={160}>
                    <SectionHeader
                        ...
                        onAction={() => router.push('/(family)/list')}
                    />
                    <ShoppingSection
                        loading={loading}
                        preview={preview}
                        ...
                        onSeeAll={() => router.push('/(family)/list')}
                    />
                </Entrance>
                ...
                <Entrance delay={220}>
                    <SectionHeader
                        ...
                        action="Flag item"
                        onAction={() => router.push('/(family)/low-stock')}
                    />
                ...
                {/* My items */}
                <Entrance delay={280}>
                    <SectionHeader title={`My Items (${myItems.length})`} />
                    <MyItemsSection items={myItems} loading={loading} />
                </Entrance>
...
const s = StyleSheet.create({
    ...
    roleText: { fontSize: 11, fontWeight: '600', color: 'rgba(255,255,255,0.85)' },
    ...
    // My items
    myItemDot:     { width: 8, height: 8, borderRadius: 4, borderWidth: 2, borderColor: BORDER, backgroundColor: 'transparent' },
    myItemDotDone: { backgroundColor: TEAL, borderColor: TEAL },
});
