...
export default function FamilyListScreen() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-bg-primary">
        ...
        >
          {FILTERS.map((f) => (
            <TouchableOpacity
              ...
              onPress={() => setFilter(f)}
            >
              <Text className={`text-[13px] font-medium ${filter === f ? 'text-white' : 'text-text-muted'}`}>
                {f}
              </Text>
            </TouchableOpacity>
          ))}
      ...
      {/* Permission note */}
      <View className="mx-5 mt-3 flex-row items-center gap-2 bg-teal-50 border border-border rounded-xl px-3 py-2">
        <Ionicons name="information-circle-outline" size={15} color="#1D9E75" />
        <Text className="flex-1 text-[12px] text-teal-600">
          Tap the checkbox to mark any item done. You can only remove items you added.
        </Text>
      </View>
        ...
        {!loading && !error && items.length === 0 && (
          <View className="items-center py-16 gap-3">
            ...
            <Text className="text-[13px] text-text-muted text-center px-8">
              Tap "Add item" to request something for the household.
            </Text>
          </View>
        )}
        ...
        {!loading && !error && filtered.map((item) => {
          ...
          return (
            ...
            >
              ...
              {/* Delete — active only on own items */}
              <TouchableOpacity
                onPress={() =>
                  isOwn
