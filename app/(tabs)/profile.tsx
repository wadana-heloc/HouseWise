  ...
  const MENU_SECTIONS: MenuSection[] = [
    ...
    },
    {
      ...
      items: [
        ...
        { icon: 'lock-closed-outline',   label: 'Change password', route: '/change-password' },
      ],
    },
  ];
    ...
    <SafeAreaView className="flex-1 bg-bg-primary">
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 20, gap: 20 }}>
        ...
        <View className="bg-white border border-border rounded-2xl p-5 items-center">
          ...
          <Text className="text-[13px] text-text-muted mt-1">Admin · Head of Household</Text>
          <TouchableOpacity
            className="mt-4 flex-row items-center gap-1.5 bg-teal-50 rounded-xl px-4 py-2"
            onPress={() => router.push('/edit-profile')}
            activeOpacity={0.7}
          >
            <Ionicons name="pencil-outline" size={15} color="#1D9E75" />
            <Text className="text-[13px] font-medium text-teal-600">Edit profile</Text>
          </TouchableOpacity>
        </View>
            ...
            <View className="bg-white border border-border rounded-xl overflow-hidden">
              {section.items.map((item, i) => (
                ...
                >
                  ...
                  {item.onPress && !item.route ? (
                    <View className="bg-teal-50 rounded-full px-2 py-0.5">
                      <Text className="text-[10px] font-medium text-teal-600">Soon</Text>
                    </View>
                  ) : (
